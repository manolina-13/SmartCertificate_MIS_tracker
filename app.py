...
def show_login_page():
    ...
    with col2:
        st.markdown("## 🔐 Login to MIS Certificate Tracker")
        st.divider()
...
def show_main_app():
    """Display the main certificate tracker application"""
        ...
        with st.form("add_cert_form"):
            ...
            if submitted:
                ...
                if not cert_no_clean:
                    ...
                    result = add_certificate(cert_no_clean, amount, issue_date, maturity_date)
                    if result["success"]:
                        st.success("✅ " + result["message"])
                        st.rerun()
                    else:
                ...
                with st.form("edit_cert_form"):
                    ...
                    if submitted_edit:
                        if not new_cert_no_clean:
                            ...
                            result = edit_certificate(key, new_cert_no_clean, new_amount, new_issue, new_maturity)
                            if result["success"]:
                                st.success("✏️ " + result["message"])
                                st.rerun()
                            else:
                                st.error("❌ " + result["message"])
...
if st.session_state.user_id:
    ...
    # Tab-like interface for login/signup
    tab_col1, tab_col2 = st.columns(2)
    
    with tab_col1:
        if st.button("🔐 Login", use_container_width=True, 
                     key="login_tab", 
                     disabled=(st.session_state.auth_tab == "Login")):
            st.session_state.auth_tab = "Login"
            st.rerun()
    ...
    with tab_col2:
        if st.button("📝 Sign Up", use_container_width=True, 
                     key="signup_tab",
                     disabled=(st.session_state.auth_tab == "Sign Up")):
            st.session_state.auth_tab = "Sign Up"
            st.rerun()
